...
The translation identifier screen (accessed through shift+D) is now the translation info screen, and now includes information about the line being executed. If a language is selected, the screen will also show the line being translated, and the text of the say statement being translated.
 ...
 can now take an ATL block, which is supplied to the execute function as a keyword argument giving an ATL transform. It's also possible to define a creator-defined statement that optionally takes an ATL block, or a block of script statements.
...
Ren'Py will load .rpe.py files from `config.renpy_base` directory and the project's game directory, and execute the file before the game starts.
...
The statement callback system () has been documented.
...
The renamed and newly-documented  variable make it possible to set the maximum texture size used by Ren'Py. This isn't useful for 2D textures, but may make sense for textures used by .
 ...
 are no longer required to have the same files as a standard Ren'Py project.
...
The new  variable lets you store information at build time, and read it back in the distributed game.
...
Containers (including fixed, hbox, vbox, side, grid, viewport, and vpgrid) now pass some transform events (hover, idle, insensitive, selected\_hover, and selected\_idle) to their children, meaning that children of a button can have their own transforms to respond to those events.
 ...
 will re-run default statements that update persistent variables, making it possible to avoid persistent becoming entirely de-initialized.
...
Self voicing now speaks screens closer to the player before those further away from the player.
 ...
 will ensure that the frames it draws are at least one pixel in size in both dimensions.
...
The default for this feature is for the `master` layer to be sticky, as well as any layers created with  unless passed the new parameter `sticky=False`.
...
The  Action now takes a delay parameter, so that the scrolling is animated over a short period of time.
...
Init statements inside a module loaded with  are now run in init-priority order.
...
There is a new "main" volume that can be accessed through `Preferences()`. The main volume is multiplied with all the other volumes to globally reduce the volume of the game.
...
The  style property now defaults to None for drag displayables. This improves performance, but means that the displayable can be dragged by transparent pixels.
...
There is a new `camera` statement, intended for use with the 3D Stage system. This statement is similar to the `show layer` statement, except that the layer name is not required, and it is not cleared when a `scene` statement is run.
...
The  class can now be used to remap characters inside a font.
...
The new  and  variables make it possible to fade out and in music when a game is loaded or other context changes cause the music to change.
...
Transforms (including ATL Transforms) now support a new  property, which either a matrix or an object that creates a matrix that changes in time, and uses it to recolor everything that is a child of the transform.

While previous versions of Ren'Py supported the  image manipulator, the new property is much improved. The image manipulator would often take a large fraction of a second, making it too slow for real-time use, and was limited to single images. The new transform property is fast enough that it can be changed every frame if necessary, and can be applied to any displayable. It's now possible to apply a Transform using matrixcolor to a layer, to recolor the entire layer - making it possible to push your game into sepia or black-and-white without needing a separate set of images.

There are a few difference between the image manipulator and the transform property versions of matrixcolor, as the new version uses 4x4 matrices and premultiplied alpha color, so the new property can't use the same matrices. Instead, there are number of new  objects that need to be used.

Transforms also support a new  property, which blurs the child of the displayable by the given number of pixels.
...
First, Ren'Py now uses  to provide standard library compatibility. It's now possible to import modules using their Python 3 names, when a renaming has occured.
...
| Platform | CPU | Note |
| --- | --- | --- |
| Linux | x86\_64 | Raised minimum version to Ubuntu 16.04 |
| Linux | i686 | Raised minimum version to Ubuntu 16.04 |
| Linux | i686 | Raised minimum version to Ubuntu 16.04 |
| Linux | armv7l | Intended to support Raspberry Pi, uses Raspian Buster |
| Windows | x86\_64 | A new port to 64-bit Windows Vista and later. |
| Windows | i686 | Raised minimum version to Windows Vista. |
| macOS | x86\_64 | macOS 10.10+ |
| Android | armv7a | Android 4.4 KitKat |
| Android | arm64 | Android 5.0 Lollipop |
| Android | x86\_64 | Android 5.0 Lollipop |
| iOS | arm64 | All 64-bit iOS devices, iOS 11.0+ |
| iOS | x86\_64 | The 64-bit iOS simulator, iOS 11.0+ |
| Web | webasm | Modern web browsers |

The biggest new platform that Ren'Py supports is the 64-bit Windows platform, which means that Ren'Py is available in 64-bits on all major desktop and mobile platforms. The new `renpy.bits` variable can be used to determine if Ren'Py is running on a 32 or 64-bit platform, if necessary. (For example, to set  appropriately.)
...
The new  variable automatically creates the steam\_appid.txt file for you. This needs to be set by a `define` statement, or in a python early block.
...
Defining a mouse cursor using  now uses SDL2's color cursor API, which generally results in hardware acceleration and reduced mouse movement lag.
...
The new  property of transforms provides for different ways of making an image fit size with a different aspect ratio. For example, it can be scaled to be contained fully within the given size, or to make sure that it completely covers the given size.

The  and  transform properties no longer double the size of the displayable they are applied to, making them easier to combine with positioning transform properties.

The  function can now take regular expressions when determining what is and is not allowed.

Grids now take  style properties, that is applied outside the grid, and inside a containing viewport.
...
The new  variable makes it easier to customize the mac app.

The  library, is bundled with Ren'Py, making accessing the web much easier.
...
The `audio` directory, which automatically defines names in the , has been made visible in the launcher, and is added to newly-created projects.

The new  callback allows an application to replace Ren'Py's exception handling system in its entirety.
...
*   On all platforms, graphical glitches could occur when  was used.
    

### Other Changes

Dynamic images can now include "\[prefix\_\]" everywhere, and especially when `add` has been used to add a dynamic image to buttons, drags, and similar focusable objects.
...
Ren'Py now supports an `audio` directory, which automatically defines names in the . This makes it possible to have a file named `game/audio/overture.ogg`, and play it using:

play music overture

The new  class allows you to provide compressed audio data to Ren'Py, either generated programatically or taken from a source other than a file. To support this, the Python wave and sunau modules are now packaged with Ren'Py.
...
*   'game.zip' can now be renamed. 'DEFAULT\_GAME\_FILENAME' in index.html controls this.
...
The  function can now be given the name of a screen that is used to prompt the user for input.
...
When a  with from\_current set to true occurs during a multi-part statement (like a menu with dialogue), control is restored to the first part of that multi-part statement (thus causing the dialogue to be displayed).
...
The  and  actions can now take other actions as arguments.

Many BarValues now take a force\_step argument, which forces changes to the bar to be rounded to the nearest step value.

 now allows the tile argument to be the string "integer", which tiles the contents of the frame an integer number of times.

 now allows the name argument to be a function or callable object when dynamic is true.
...
The default property of most displayables, which is used to choose a displayable to be focused by default, has been renamed to default\_focus so that it does not conflict with the `default` statement. It also now takes an integer, with the displayable with the highest focus number getting focus.

The  displayable now passes positions from the child.
...
The , which was not working properly in most circumstances, has been fixed.
...
The  function has new arguments to enable new functionality.

*   Statement prediction can be controlled by the predict\_all and predict\_next arguments, which predict all possible next statements or take a function that determines what will run next, respectively.
    
*   The new post\_execute argument lets one specify an execute function that is run as the next statement - the one after the creator-defined statement. This allows a pattern where a statement runs, executes the block inside it, and then runs something after the block to clean it up. (For example, an event that serves as a label, and then jumps back to a dispatcher when it is done.)
    
*   The new post\_label argument lets one specify a function to supply the a label that goes after the creator-defined statement, which can function like the `from` clause to the call statement.
...
Ren'Py now includes support for self-closing custom text tags, which are  that do not require as closing text tag.
...
Ren'Py now uses Framebuffer Objects on any device that claims to support it. As a result  now works on Android and iOS.
...
The  and  functions now take a zorder argument.
...
The new  specifies a label that is jumped to when a load fails because Ren'Py can no longer find the current statement. This makes it possible to a game to implement its own recovery mechanism.

The new  variable makes it possible to intercept the notification system and do your own thing.

The interface of  has been changed in an incompatible way, to allow sets of old and new tags to be given.

 and  now support a get\_tooltip method, which allows the object to supply a default tooltip.
...
The bar values , , , and  all take a new action parameter, which gives an action that is performed each time the value changes.
...
The  function returns True when in the rollback that occurs immediately after a load. This makes it possible to use:

python:
    if not renpy.in\_rollback():
        renpy.run(ShowMenu('save'))

To display a save menu during an initial playthough, but not during loading or rollback.

 now exists, making it possible to select the transition to use on a per-say-statement basis.

The new `RENPY_SEARCHPATH` environment variable makes it possible to override  on launch.
...
A crash when  is true on macOS has been fixed.

A crash when  is true has been fixed.
...
Ren'Py now has support for . Arguments can be passed to a choice menu as a whole, or to the individual choices within the menu, using the syntax:
...
Menu arguments passed to the menu itself become arguments to the screen, except the screen argument which selects the screen, and the nvl argument, which - if present - selects the NVL-mode menu. The arguments to the choices become arguments to the items passed to the menu screen.
...
*   It is now possible to choose how the outline scales when the window is scaled. This is done with the  style property.
    

When positioning a Text object, the  property can be renpy.BASELINE. When it is, the anchor is set to the baseline of the first line of the text.

### Statements

The new `window auto show` and `window auto hide` statements allow  to continue while showing or hiding the dialogue window.

The `show screen` and `hide screen` statements now take a with clause, that works the same way it does with `show` and `hide`.

The screen language `use` statement now can take an `expression` clause, that makes it take an expression rather than a literal screen name. This allows a variable to be used when selecting the screen that is included. See  for more details.

### Changes

The new  function reports if Ren'Py is currently skipping.

The  displayable now takes a new copypaste property, which when true allows copying with ctrl+C and pasting with ctrl+V. This is enabled in the console and launcher.

`Preference("display", "window")` now avoids creating a window bigger than the screen, and will be selected if the current window size is the maximum window size, if the size selected with  is bigger than the maximum window size.

 now have a few more lexer methods available, making it possible to to parse arguments, image name components, labels, and delimited python.

The  function takes a new argument, that causes the autosave to block until it completes.

The  now takes a number of new parameters, if required.

The new  image manipulator can blur an image. Thanks to Mal Graty for contributing it.
...
The  displayable can now be told not to loop, and displays the associated static image when it stops looping. It also can take an image that is displayed before the movie proper starts.
...
Size-changing properties like  now apply to a Drag and not the space it can move around in.
...
Ren'Py will no longer crash if an incomparable type is given to  and other actions.
...
This was a quick re-release of 7.1.2 in order to fix a single bug, which was that a test change had been left in causing  to be set on initial startup.
...
The new  and  actions make it possible to set variables inside used screens.

The new  variable determines if menus should include entries disabled by an if clause.
...
The new  variable indicates the version of the game itself that was used when a new game is first created. This only stores the version at game creation - after that, it's up to the creator to keep it updated.

The  displayable now supports a new mode the color data and alpha mask data are placed side-by-side in the same file. This prevents issues where a main and mask movie could go out of sync.

The  and  functions now take arguments that control if the they can bring the player to the auto or quick save pages.

The new  variables determines if Ren'Py will skip non-looping audio files that are played while Ren'Py is skipping.

### Translations

Ren'Py now has the ability to automatically detect the locale of the user's system, and use it to set the language. Please see  and the  documentation for how this works.
...
screen history():
    ...
    use game\_menu(\_("History"), scroll\=("vpgrid" if gui.history\_height else "viewport"), yinitial\=1.0):

        style\_prefix "history"

        for h in \_history\_list:

            window:

                \## This lays things out properly if history\_height is None.
                has fixed:
                    yfit True
                ...
                if h.who:
                    ...
                    label h.who:
                        ...
                        if "color" in h.who\_args:
                            text\_color h.who\_args\["color"\]

                $ what \= renpy.filter\_text\_tags(h.what, allow\=gui.history\_allow\_tags)
                text what substitute False

        if not \_history\_list:
...
Ren'Py gives a different numeric version number to the x86\_64 apk. This will allow both x86\_64 and armeabi-v7a builds to be uploaded to Google Play and other stores, rather than having to first created one build and then the other, manually changing the version numbers between.
...
Ren'Py supports the a new "nestled-close" value for the ctc\_position parameter of . This value prevents there from being a break between the click-to-continue indicator and the other lines.

Drags (in drag-and-drop) now support alternate clicks. (Right clicks on desktop and long-clicks on touch platforms.)

### Fixes

The  and  functions have been extended to accept namespaces and fields. So it's now possible to have actions like `SetVariable("hero.strength", hero.strength + 1)` or `ToggleVariable("persistent.alternate_perspective")`.
...
The x86\_64 architecture has been added, while x86 has been dropped. (Some x86 devices may be able to run the arm platform version through binary translation layers.)
...
will create two blocks of dialogue. See  for more info.

There is also a new {clear} text tag that works with monologue. When the {clear} tag is part of a line by itself, it is the equivalent of the `nvl clear` statement. See  for more about this.
...
Ren'Py can now automatically save the game upon quit, in a reliable fashion. (As compared to the previous autosave, which could fail or be cycled out.) This is controlled by the  variable.

File actions (like , , and ) can now take a slot=True argument. When this is given, the action loads a named slot, without involving the page.
...
Added , a function to transform text without touching text tags or interpolation.
...
The new  allows control of the spacing between rows or columns introduced by .

The  style property can now take "horizontal" and "vertical" as values, causing text spacing to be adjusted in only the specified direction.

LayerdImageProxy can now take an interpolated string.

The new  is called when starting the game or entering a new context, like a menu context. It can be used to stop voice or sounds from playing when entering that context.

The  displayable (and the screen language equivalent, `drag`) have grown a new activated property. This is callback that is called when the user first clicks the mouse on a drag. (Before it starts moving.)
...
The new  action makes it possible to have buttons that change the position of a viewport or the value of a bar.

The , , and  transitions now respect the alpha channels of their source displayables, as if given the `alpha=True` argument. As omitting the alpha channel is no longer an optimization, this change allows the same transitions to be used in more places.

Automatic image definitions now take place at init level 0, rather than an init level of greater than 999. This allows  to be used in `init` blocks.
...
The  statement now takes an index clause:
...
The new  system make it possible to define your own displayables that can be accessed using strings, the same way that images, image files, and solids have a string form.
...
The new  function returns the set of tags that have attributes but or otherwise hidden, while the  function can return a list of tags in sorted order.
...
The ,  and  displayables are now actually available under their new names.
...
The changelogs for some Ren'Py 6 versions can be found , with older changelogs being found at:
